...
st.dataframe(filtered_map_data)

# test_data = {
#     'latitude': [40.0209, 40.0211, 40.0199],
#     'longitude': [-75.3137, -75.3129, -75.3157],
#     'name': ['Bryn Mawr College', 'Strawberry Mansion', 'Wynnewood']
# }
...
# def create_test_map(center_lat, center_lon):
#     layer = pdk.Layer(
#         'ScatterplotLayer',
#         data=df,
...
# # Create the map
# map_center = (40.0209, -75.3137)  # Bryn Mawr College
# map = create_test_map(map_center[0], map_center[1])
...
#     # Set up last object clicked
#     last = st.session_state.get("last_object_clicked")
#     if last is not None:
...
#     clicked_row_melted = filtered_map_data[
#         (filtered_map_data["latitude"]==lat) &
#         (filtered_map_data["longitude"]==lon)
#     ]

# # Display the filtered result
# if not clicked_row_melted.empty:
#     st.dataframe(clicked_row_melted, width='stretch')
# else:
#     st.info("No matching event found for the clicked location.")
...
if len(filtered_map_data) > 0:
	...
	st_data = st.folium(m, width = 725, key = "main_map")
